...
#Guidelines to be followed while scripting

>>	Proper documentation has to be maintained ie
    At test case level, the testcase objective, steps,checks included and the created by info has to be updated. Also appropriate comments is required to specify the action of the code snippets in the testcase
    At the  page level, one liner about each page is required

>>  Locators should be chosen as per specified priority
    ID -> CSS selectors -> Xpath
    For link, linktext -> CSS Selectors -> Xpath

>>  The code snippets with respect to page objects has to be maintained in appropriate page classes

>>  The reusable major business flows (application specific) has to be maintained under Business Layer folder

>>  Generic reusable functions(across Pega applications)should be maintained under Pegaspecific folder

>>  Locators for the objects have to be maintained in the Locators.properties file

>>  All the global variables(Application-specific) has to be maintained in the Project.properties file

>>  Runtime values can be put to Data.properties which is later deleted after every suite execution
...
>> ingle Responsibility Principle - A component of code (e.g. class or function) should perform a single well defined task. http://en.wikipedia.org/wiki/Single_responsibility_principle
